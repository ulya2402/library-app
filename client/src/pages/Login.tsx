...
export default function Login() {
  ...
  const handleFaceCaptured = async (blob: Blob) => {
    ...
    try {
      ...
      setIsVerifying(false); setIsCameraActive(false); 
    }
  };
  ...
  return (
    <motion.div variants={pageTransition} initial="hidden" animate="show" exit="exit" className="min-h-screen flex items-center justify-center p-4 sm:p-8 bg-[#FAF9F6]">
      <motion.div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-6 sm:p-8 w-full max-w-[400px] relative overflow-hidden">
        <button type="button" onClick={handleDummyLogin} className="absolute top-6 right-6 p-2 bg-gray-50 text-gray-400 hover:text-blue-600 rounded-full z-10"><Bug className="w-4 h-4" /></button>
        <div className="text-center space-y-2 mb-8 mt-2">
          <div className="mx-auto w-12 h-12 bg-gray-50 rounded-2xl flex items-center justify-center mb-4"><ScanFace className="w-6 h-6 text-gray-900" /></div>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="scanner" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-gray-50/30 p-2 rounded-2xl border border-gray-100">
              {!isCameraActive ? (
                <div className="w-full aspect-[4/3] flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
                  ...
                  <button onClick={() => setIsCameraActive(true)} className="py-2.5 px-5 bg-gray-900 text-white rounded-xl text-xs font-bold hover:bg-black transition-all shadow-lg active:scale-95">Activate Camera</button>
                </div>
              ) : (
          ...
          {isVerifying && (
            <motion.div key="verifying" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center py-12 space-y-4">
              <Loader2 className="w-10 h-10 text-gray-900 animate-spin" />
              <p className="text-sm font-bold text-gray-900">Analyzing via Luxand AI...</p>
            </motion.div>
          )}
            ...
            <motion.div key="result" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center py-8 space-y-6 text-center">
              {result.success ? (
                <><motion.div initial={{ scale: 0 }} animate={{ scale: 1, transition: { type: "spring", bounce: 0.5 } }}><CheckCircle2 className="w-16 h-16 text-green-500" /></motion.div>
                  <div className="space-y-1"><p className="text-sm font-bold text-green-600 uppercase tracking-widest">{result.message}</p><p className="text-2xl font-bold text-gray-900">{result.name}</p></div>
                  <button onClick={handleEnterDashboard} className="mt-4 w-full flex items-center justify-center gap-2 py-3.5 px-4 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-black transition-colors">Enter Dashboard <ArrowRight className="w-4 h-4" /></button>
                </>
              ) : (
                <><motion.div initial={{ scale: 0 }} animate={{ scale: 1, transition: { type: "spring", bounce: 0.5 } }}><XCircle className="w-16 h-16 text-red-500" /></motion.div>
                  <div className="space-y-1"><p className="text-lg font-bold text-gray-900">Access Denied</p><p className="text-sm font-medium text-gray-500">{result.message}</p></div>
                  <button onClick={() => { setResult(null); setIsCameraActive(false); }} className="mt-4 w-full py-3.5 px-4 bg-gray-100 text-gray-900 rounded-xl text-sm font-bold hover:bg-gray-200 transition-colors">Try Again</button>
                </>
              )}
            </motion.div>
          )}
